...
class Document:
    def __init__(self, device='wired', data=None):
        self.device = device  # Device.where('name', '=', device).get()[0]
        ...
        if data is None:
            ...
            self.get_data(length=4000)
        else:
            self.data = ColumnDataSource(data=data)

    def get_data(self, length=2000):
        device_ = Device.where('name', '=', self.device).get()[0]
        db_data = device_.datas().order_by('id', 'asc').get()
...
def get_graph_components(data=None):
